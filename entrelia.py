...
if check_password():
    ...
    # Aseguramos normalización de columnas críticas antes de operar
    ...
    with st.sidebar:
        ...
        menu = st.selectbox("Módulo de Gestión", modulos_visibles)
    ...
    if menu == "🏠 Dashboard":
        ...
        if col_g.button("➕ Registrar Gasto Rápido", use_container_width=True):
            st.session_state["accion_rapida"] = "gasto"
            st.rerun()

        if col_i.button("➕ Registrar Ingreso Rápido", use_container_width=True):
