...
if check_password():
    ...
    st.markdown("""
        ...
        /* Botón hamburger propio — solo visible en móvil */
        #evoq-menu-btn { display: none; }
        ...
        <button id="evoq-menu-btn" onclick="
            try {
                var doc = document;
                var sidebar = doc.querySelector('[data-testid=stSidebar]');
                var trigger = doc.querySelector('[data-testid=collapsedControl]');
                if (trigger) {
                    trigger.click();
                } else if (sidebar) {
                    var expanded = sidebar.getAttribute('aria-expanded');
                    if (expanded === 'false' || sidebar.style.display === 'none') {
