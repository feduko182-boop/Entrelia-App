...
if check_password():
    ...
    # --- MÓDULO: ANULACIÓN ---
        ...
        if not df_master.empty:
            ...
            movs = df_anula[df_anula['Estado'] != 'Anulado'].copy()
            if not movs.empty:
                movs['Display'] = movs['Fecha'].astype(str) + " | " + movs['Obra'] + " | $" + movs['Monto'].astype(str)
                sel = st.selectbox("Movimiento a anular:", movs['Display'])
                motivo = st.text_input("Motivo:")
                if st.button("Confirmar Anulación") and motivo:
                    idx = movs[movs['Display'] == sel].index
                    df_anula.loc[idx, 'Estado'] = 'Anulado'
                    df_anula.loc[idx, 'Detalle'] += f" [ANULADO: {motivo}]"
