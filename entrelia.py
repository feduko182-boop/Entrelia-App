...
if check_password():
    ...
    if not st.session_state["onboarding_visto"]:
        ...
        # ── Ocultar sidebar durante onboarding ───────────────────────────
        st.markdown("""
            <style>
        ...
        st.markdown("""
            ...
            <h1 style="text-align:center; color:#1a73e8; margin-bottom:4px;">
                Bienvenido a Evo Q
            </h1>
            <p style="text-align:center; color:#555; font-size:15px; margin-bottom:32px;">
                Tu sistema de gestión financiera para obras.<br>
                En menos de 1 minuto estás operando.
            </p>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
            ...
            ">
                <p style="font-weight:600; font-size:15px;
                          color:#1a73e8; margin-bottom:12px;">
                    ¿Por dónde empiezo?
                </p>
                <ul style="font-size:15px; color:#333;
                           line-height:2.2; margin:0; padding-left:20px;">
                    <li>🏗️ <b>Crea tu primera obra</b>
                        desde el Dashboard (botón acción rápida)</li>
                    <li>📂 <b>Carga tu presupuesto OPUS</b>
                        en "Obra y Documentos"</li>
                    <li>📝 <b>Registra tu primer gasto o ingreso</b>
                        en Registro Contable</li>
                </ul>
                <p style="font-size:13px; color:#777;
                          margin-top:12px; margin-bottom:0;">
                    💡 Con esto tendrás rentabilidad, inflación
                    y proyección de cierre en tiempo real.
                </p>
            </div>
        """, unsafe_allow_html=True)

        # ── Video tutorial (placeholder — reemplazar URL cuando esté listo)
        st.markdown("#### 🎥 Tutorial rápido")
        VIDEO_URL = "https://www.youtube.com/watch?v=dQw4w9WgXcQ"  # ← reemplazar
        try:
            st.video(VIDEO_URL)
        except Exception:
            st.info("📹 Video tutorial próximamente disponible.")

        st.markdown("<br>", unsafe_allow_html=True)

        # ── Botones de acción ─────────────────────────────────────────────
        col_l, col_ok, col_skip, col_r = st.columns([2, 3, 2, 2])
        ...
        # ── BLOQUEO TOTAL — nada del sistema se renderiza hasta aquí ──────
        st.stop()
