...
def check_password():
    """Devuelve True si el usuario ingresó usuario y contraseña correctos."""
    if "password_correct" not in st.session_state:
        ...
        with col2:
            # --- AQUÍ ESTÁ EL CAMBIO: DOS CAMPOS ---
            user_input = st.text_input("Usuario")
            pwd_input = st.text_input("Contraseña", type="password")
            
            if st.button("Ingresar al Sistema"):
                # Traemos la lista de usuarios del Excel
                df_u = consultar_datos("Usuarios")
                
                if not df_u.empty:
                    # Buscamos si existe esa combinación en el Excel
                    # IMPORTANTE: Revisa que tus columnas en Excel se llamen USUARIO y PASSWORD
                    valido = df_u[(df_u['USUARIO'] == user_input) & (df_u['PASSWORD'] == pwd_input)]
                    
                    if not valido.empty:
                        st.session_state["password_correct"] = True
                        # Guardamos el nombre real (ej: Hugo) para el saludo
...
if check_password():
   ...
   # --- 4. MENÚ LATERAL ---
    with st.sidebar:
        ...
        st.title(f"Hola, {st.session_state.get('usuario_actual', 'Usuario')} 👋") # Saludo personalizado
    ...
    if menu == "📊 Salud Financiera":
        ...
        st.write(f"**Usuario Actual:** {st.session_state.get('usuario_actual', 'Admin')}")
        st.divider()
        
        with st.expander("👥 Gestión de Acceso - Crear Nuevo Usuario"):
            with st.form("crear_user_final", clear_on_submit=True):
                u_nombre = st.text_input("Nombre Real (ej: Hugo)")
                u_login = st.text_input("Nombre de Usuario (para entrar)")
                u_pass = st.text_input("Contraseña", type="password")
                u_rol = st.selectbox("Nivel de Acceso", ["admin", "operador"])
                
                if st.form_submit_button("🔨 Registrar en Base de Datos"):
                    if u_nombre and u_login and u_pass:
                        try:
                            # 1. Leemos los usuarios que ya existen
                            df_usuarios_act = consultar_datos("Usuarios")
                            
                            # 2. Creamos el nuevo registro con tus columnas exactas
                            ...
                            # 3. Unimos y subimos a la nube
                            df_final_u = pd.concat([df_usuarios_act, nuevo_u], ignore_index=True)
                            conn.update(worksheet="Usuarios", data=df_final_u)
                            
                            st.success(f"✅ ¡Usuario '{u_nombre}' guardado exitosamente en el Excel!")
                            st.balloons()
