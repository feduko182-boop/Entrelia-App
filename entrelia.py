...
if check_password():
    ...
    # --- MÓDULO: COMPARADOR DE PRECIOS ---
        ...
        try:
            # Mantenemos tu conn.read, pero le pasamos la URL completa para matar el error de 'None'
            url_maestra = "https://docs.google.com/spreadsheets/d/1fpTC_6y9fndidwnr4xaZf2ISDRGNKHEFYW-OLwxMzt0"
            df_p = conn.read(spreadsheet=url_maestra, worksheet="Catalogo_Precios", ttl=0)
            
            if not df_p.empty:
                df_p.columns = df_p.columns.str.strip() 
                
                st.subheader("🔍 Buscar el mejor precio")
                # ... aquí sigue tu lógica de mat_list y selectbox ...
