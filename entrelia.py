    ...
    if "password_correct" not in st.session_state:
        ...
        with col2:
            ...
            if st.button("Ingresar al Sistema"):
                ...
                if not df_u.empty:
                    # Filtramos ignorando espacios en blanco para evitar errores de tipeo
...
if check_password():
    ...
    # --- MÓDULO: ANULACIÓN ---
        ...
        if not df_master.empty:
            ...
            # Filtramos los que están activos para poder anularlos
            ...
            if not movs_activos.empty:
                ...
                if st.button("Confirmar Anulación") and motivo:
                    ...
                    # Actualizamos el estado y enriquecemos el detalle con la metadata de anulación
                    # Nota: Aquí usamos 'Admin' como usuario por defecto, puedes cambiarlo por st.session_state.user si tienes login
                    df_anula.loc[idx, 'Estado'] = 'Anulado'
                    df_anula.loc[idx, 'Detalle'] = df_anula.loc[idx, 'Detalle'] + f" | [ANULADO el {ahora} por Admin. Motivo: {motivo}]"
                    
                    conn.update(data=df_anula)
                    st.success("Movimiento anulado correctamente.")
                    st.rerun()
            else:
            ...
            # --- NUEVA SECCIÓN: HISTORIAL DE ANULACIONES ---
            st.subheader("📜 Historial de Movimientos Anulados")
            df_historial = df_anula[df_anula['Estado'] == 'Anulado'].copy()
            
            if not df_historial.empty:
                # Limpiamos la vista para el usuario
                historial_display = df_historial[['Fecha', 'Obra', 'Monto', 'Tipo', 'Detalle']].copy()
                historial_display.columns = ['Fecha Origen', 'Obra', 'Monto Anulado', 'Categoría', 'Registro de Anulación']
                
                st.dataframe(historial_display, use_container_width=True)
                
                # Resumen rápido para el análisis de tu papá
                total_anulado = historial_display['Monto Anulado'].sum()
                st.warning(f"⚠️ Total de flujo anulado: ${total_anulado:,.2f} MXN")
            else:
                st.write("No hay registros anulados todavía.")
