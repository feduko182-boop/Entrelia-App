...
if check_password():
    ...
    if "onboarding_visto" not in st.session_state:
        # Revisamos si ya lo vio antes (persistido en URL igual que la sesión)
        ya_visto_url = st.query_params.get("onboarding_done", "false") == "true"
        st.session_state["onboarding_visto"] = ya_visto_url
    ...
    if not st.session_state["onboarding_visto"]:
        ...
        # ── Restaurar sidebar al salir del onboarding ─────────────────
    else:
        st.markdown("""
            <style>
        ...
        with col_ok:
            if st.button("✅ Continuar", key="onboarding_ok", use_container_width=True):
                ...
                st.query_params["onboarding_done"] = "true"  # persiste en URL
                st.rerun()
    ...
    st.markdown("""
        ...
        /* Ocultar toolbar flotante y decoraciones de Streamlit en móvil */
        ...
        if archivo_ticket:
            ...
            if st.button("🕵️ Escanear con IA"):
                with st.spinner("🤖 Evo Q está leyendo tu ticket..."):
                    try:
                        model_ia = genai.GenerativeModel('gemini-pro-vision')
                        prompt_ia = (
