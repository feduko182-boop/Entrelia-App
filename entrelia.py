...
def check_password():
    ...
    if "session_active" in st.query_params and st.query_params["session_active"] == "true":
        ...
        st.session_state["user_id"] = st.query_params.get("uid", "default")
        return True
            ...
            if st.button("Ingresar al Sistema"):
                ...
                if not df_u.empty and 'USUARIO' in df_u.columns and 'PASSWORD' in df_u.columns:
                    ...
                    if not valido.empty:
                        ...
                        user_id_final = str(valido.iloc[0]['USUARIO']).strip()
                        st.session_state["password_correct"] = True
...
if check_password():
    ...
    if _uid and "user_id" in df_master.columns:
        df_master = df_master[df_master["user_id"].astype(str).str.strip() == _uid]
