...
if check_password():
    ...
    st.markdown("""
        ...
        @media (min-width: 769px) {
            div[data-testid="stHorizontalBlock"]:has(button[key="evoq-sidebar-toggle"]),
            div[data-testid="stHorizontalBlock"]:has(#evoq-sidebar-toggle) {
                display: none !important;
            }
        }

        /* Botón hamburger — visible en móvil sin romper el layout */
        @media (max-width: 768px) {
            ...
            /* El sidebar en móvil es overlay nativo — no interferir con su transform */
    ...
    if "sidebar_abierto" not in st.session_state:
        st.session_state["sidebar_abierto"] = False  # cerrado por defecto en móvil

    col_ham, _ = st.columns([1, 11])
    with col_ham:
        if st.button("☰", key="evoq-sidebar-toggle"):
            st.session_state["sidebar_abierto"] = not st.session_state.get("sidebar_abierto", False)

    # Controlar visibilidad del sidebar según estado — solo en móvil
    if not st.session_state.get("sidebar_abierto", False):
        st.markdown("""
            <style>
            @media (max-width: 768px) {
                [data-testid="stSidebar"] {
